...
и модели `country` (модель страны). После чего, с помощью репозитория модели,
в коллекцию добавляется новый документ (страна), который выводится в консоль.
...
В следующем блоке определяется модель `city` (модель города) со связью
`belongsTo` к модели `country` (модель страны). Затем создается новый документ
города, связанный с ранее созданной страной из примера выше. После создания
нового документа, выполняется запрос на извлечение данного города с включением
связанной страны.
