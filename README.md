...
Модель данных описывает структуру документа определенной коллекции.
Экземпляр класса `Schema` имеет метод `defineModel` с помощью которого
объявляется новая модель.
...
то указывая название источника в параметре `datasource` появляется
