...
- **Схема баз данных** (*DatabaseSchema*) - хранилище определений источников данных и
моделей. Связывает модели с источниками данных и предоставляет доступ к
экземплярам репозиториев.

- **Источник данных** (*Datasource*) - конфигурация подключения к базе данных.
Содержит название используемого адаптера и параметры соединения.

- **Адаптер** (*Adapter*) - низкоуровневая реализация для взаимодействия с
конкретной СУБД. Выполняет непосредственные операции чтения и записи
данных коллекции.
...
Пример демонстрирует создание экземпляра `DatabaseSchema` (схемы баз данных),
объявление источника данных `myDb` и модели `country`. После чего, с помощью
репозитория данной модели, в коллекцию добавляется новый документ, который
выводится в консоль.
