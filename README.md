...
Перед началом работы с коллекциями потребуется выполнить настройку проекта,
которая заключается в определении источников и моделей данных. Определения
хранятся в экземпляре класса `Schema`, и первым шагом будет создание данного
экземпляра.
