...
dbs.defineModel({
  ...
  properties: {
    ...
    // рекомендуется указать, так как адаптер "memory"
    // по умолчанию создает числовые идентификаторы
  },
  relations: {
    // объявление связи "country" позволит автоматически включать
    // связанные документы с помощью опции "include" при обращении
    // к данной коллекции через методы репозитория
    country: {
